...
public class ViewModel extends Observable implements Observer {
    ...
    public void findPath(final double height, double width) {
        if (!path.getValue()) {
            model.connectPath(ip.getValue(), Integer.parseInt(port.getValue()));
        }
        model.findPath((int) (airplaneY.getValue() / -1), (int) (airplaneX.getValue() + 15), Math.abs((int) (markSceneY.getValue() / height)),
                Math.abs((int) (markSceneX.getValue() / width)), data);
    }
